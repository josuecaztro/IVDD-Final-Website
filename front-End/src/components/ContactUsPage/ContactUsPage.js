...
import contactText from './ContactUsTEXT';



function DeployContactUs( { language } ){
...
//testing a comment
    ...
    const [formData, setFormData] = useState({
      ...
      const handleFormSubmit = (e) => {
        ...
        fetch('https://iglesiavozdedios.net:8080/api/contact-requests', {
          method: 'POST',
          headers: {
            'Accept': '*/*',
        ...
        .then(() => {
          alert(contactText[language].successMessage);
          setFormData({
            firstName: '',
            lastName: '',
            phoneNumber: '',
        ...
        .catch(error => {
          ...
          alert(contactText[language].errorMessage);
        });
      };
    ...
    return (
        ...
        <p id="bible-quote" className="text-center">
          ...
          &nbsp;{contactText[language].bibleQuote}
        </p>
        ...
        <form id="contact-form-id" className="p-4 rounded-3 shadow-sm bg-light" onSubmit={handleFormSubmit} autoComplete="off">
          ...
          <button type="submit" className="btn btn-primary w-100">{contactText[language].submit}</button>
        </form>
      </div>
      </div>
    )
